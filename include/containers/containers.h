 ...
 */
inline void container_to_uint32_array(uint32_t *output, void *container,
                                      uint8_t typecode, uint32_t base) {
    switch (typecode) {
        case BITSET_CONTAINER_TYPE_CODE:
            bitset_container_to_uint32_array(output, container, base);
            break;
        case ARRAY_CONTAINER_TYPE_CODE:
            array_container_to_uint32_array(output, container, base);
            break;
        case RUN_CONTAINER_TYPE_CODE:
            run_container_to_uint32_array(output, container, base);
            break;
            //  case UNINITIALIZED_TYPE_CODE: break;
    }
}
...
inline void *container_and(void *c1, uint8_t type1, void *c2, uint8_t type2,
    ...
    switch (type1 * 4 + type2) {
        case (BITSET_CONTAINER_TYPE_CODE * 4 + BITSET_CONTAINER_TYPE_CODE):
            ...
            // temp temp, type signature is to return an int, destination param
            // is third
