...
bool array_container_intersect(const array_container_t *array1,
    ...
    if (card_1 * threshold < card_2) {
        ...
        /* Balanced-merge nonempty: SIMD path matches cardinality/intersect
         * kernels (wasm digest parity); historically scalar-only Nonempty left
         * room—see intersect_vector16_nonempty docstring for perf caveat. */
#if CROARING_IS_X64
        if (croaring_hardware_support() & ROARING_SUPPORTS_AVX2) {
