...
static int metamorphic_or_chain_fuzz(const uint8_t *data, size_t size) {
    ...
    if (linear == NULL || bal == NULL || heap_u == NULL || many_u == NULL) {
        /* Allocation failure is not a bug: clean up and skip this input
         * rather than abort()ing (which libFuzzer would report as a crash). */
...
static int metamorphic_bitmap_binary_ops_fuzz(const uint8_t *data,
    ...
    if (out_or == NULL || out_and == NULL || out_xor == NULL ||
        ...
        /* Allocation failure is not a bug: free and skip. */
...
int LLVMFuzzerTestOneInput(const char *data, size_t size) {
    ...
    /* Keep the historical (de)serialization fuzzing as the dominant path so
     * existing OSS-Fuzz coverage and corpus are not diluted: bitmap32 and
     * bitmap64 stay balanced (matching the old data[0] % 2 split) and together
     * take 6/8 of inputs; the two metamorphic harnesses share the other 2/8. */
