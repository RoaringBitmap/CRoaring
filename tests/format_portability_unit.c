...
inline long filesize(char const* path) {
...
inline char* readfile(char const* path) {
    FILE* fp = fopen(path, "rb");
    assert_non_null(fp);
...
inline int compare(char* x, char* y, size_t size) {
...
inline void test_deserialize(char* filename) {
