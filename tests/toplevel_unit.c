    ...
    for (int card = 2; card < 1000000; card *= 8) {
        ...
        for (int i = 0; i < card; ++i) {
            ...
            uint32_t pos = (uint32_t)(
                f1 * f2 * f3 * range);  // denser at the start, sparser at end
            assert(pos < range);
            roaring_bitmap_add(r, pos);
            input[pos] = 1;
        }
...
void test_iterator_reuse_retry_count(int retry_count) {
    ...
    assert(roaring_bitmap_get_cardinality(with_edges) - 2 ==
...
DEFINE_TEST(test_range_cardinality) {
    ...
    assert(roaring_bitmap_range_cardinality(r, s * 2 + 100, s * 3 - 200) ==
           s - 300);

    // multiple containers (maxhb > minhb)
    assert(roaring_bitmap_range_cardinality(r, s * 2, s * 5) == s * 3);
    assert(roaring_bitmap_range_cardinality(r, s * 2 + 100, s * 5) ==
           s * 3 - 100);
    assert(roaring_bitmap_range_cardinality(r, s * 2, s * 5 - 200) ==
           s * 3 - 200);
    assert(roaring_bitmap_range_cardinality(r, s * 2 + 100, s * 5 - 200) ==
           s * 3 - 300);
