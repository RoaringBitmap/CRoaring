 ...
 * export; cardinality and maximum agree with the export.
 *
 * Note: this is a one-directional ("no phantom members" + counts) oracle. It
 * does not assert the converse -- that every predicate-true element is present
 * -- so an op that silently drops a true element is caught not here but by the
 * cardinality identities (ca+cb == cor+cand, cxor+cand == cor) and the
 * inplace-vs-fresh equality checks in the metamorphic tests below. */
...
DEFINE_TEST(portable_roundtrip_equals_and_matches_or_membership_oracle) {
    ...
    char *serialized = (char *)calloc(sz ? sz : 1u, 1);
